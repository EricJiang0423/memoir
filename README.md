...
**本地优先，但 AI 需要看照片。** EXIF 解析、图片缩放、聚类、base64 编码、HTML 后处理均在本地完成。GPS 坐标（两位小数精度）发往 OpenStreetMap Nominatim 做地名查询。**照片会传给多模态大模型**（如 Claude）做视觉策展和叙述写作——这是流水线的必要环节，没有它就无法筛选照片、判断构图、写出有意义的 caption。照片**不会上传到图床、不会用于模型训练、不会进入任何第三方存储**。
