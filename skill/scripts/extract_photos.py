...
def scan_photosapp(album=None, date_range=None):
    ...
    for idx, p in enumerate(photos, start=1):
        ...
        # Apple Photos native scores (only available in Photos.app mode)
        apple_score = None
        try:
            s = p.score
            if s and s.overall > 0:
                apple_score = {
                    "overall": round(s.overall, 4),
                    "curation": round(s.curation, 4),
                    "promotion": round(s.promotion, 4),
                    "highlight_visibility": round(s.highlight_visibility, 4),
                    "pleasant_composition": round(s.pleasant_composition, 4),
                    "sharply_focused_subject": round(s.sharply_focused_subject, 4),
                    "well_framed_subject": round(s.well_framed_subject, 4),
                    "well_timed_shot": round(s.well_timed_shot, 4),
                    "interesting_subject": round(s.interesting_subject, 4),
                    "pleasant_lighting": round(s.pleasant_lighting, 4),
                    "harmonious_color": round(s.harmonious_color, 4),
                    "noise": round(s.noise, 4),
                    "failure": round(s.failure, 4),
                }
        except Exception:
            pass
        ...
        try:
            for person in p.persons:
                persons.append(person if isinstance(person, str) else person.name)
        except Exception:
            pass
        ...
        records.append({
            ...
            "is_screenshot": bool(p.screenshot),
            "is_favorite": bool(p.favorite),
            "is_hidden": bool(p.hidden),
            "persons": persons,
        })
    return records
