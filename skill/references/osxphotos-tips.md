...
## iCloud 下载权限问题（macOS 26+ / Ghostty / cmux）

当 terminal app 没有「完全磁盘访问」或 Photos 权限时，osxphotos 的 `--download-missing` 和 `--use-photokit` 都会失败。

### 症状

```
Error: could not get authorization to access Photos library
Please ensure that your terminal app (ghostty) is granted access in
'System Settings > Privacy & Security > Photos'
```

### 原因

macOS TCC（Transparency, Consent, and Control）框架阻止终端进程读取 Photos.sqlite 或调用 PhotoKit API。即使 `extract_photos.py` 可以读取元数据（SQLite 直读），`export` / `download_missing` 仍需额外授权。

### 解决：AppleScript 导出（已验证可用）

绕过 osxphotos 的权限检查，直接用 AppleScript 让 Photos.app（天然有权限）自行导出照片：

```python
import osxphotos, subprocess
from pathlib import Path

db = osxphotos.PhotosDB()
photos = db.photos()

# 1. 用 osxphotos 读元数据（不需要特殊权限）
target_uuids = [p.uuid for p in photos
                if p.date and p.date.date().isoformat() in target_dates
                and p.iscloudasset]

# 2. 用 AppleScript 批量导出（Photos.app 自己有权限）
EXPORT_DIR = Path("/tmp/photos_export")
EXPORT_DIR.mkdir(parents=True, exist_ok=True)

BATCH_SIZE = 50
for i in range(0, len(target_uuids), BATCH_SIZE):
    batch = target_uuids[i:i+BATCH_SIZE]
    uuid_list = ", ".join(f'"{u}"' for u in batch)

    script = f'''
    tell application "Photos"
      set photoUUIDs to {{{uuid_list}}}
      set foundPhotos to {{}}
      repeat with aUUID in photoUUIDs
        try
          copy (media item id aUUID) to end of foundPhotos
        end try
      end repeat
      if (count of foundPhotos) > 0 then
        export foundPhotos to ("{EXPORT_DIR}" as POSIX file) with using originals
      end if
    end tell
    '''
    subprocess.run(['osascript', '-e', script], capture_output=True, text=True, timeout=300)
