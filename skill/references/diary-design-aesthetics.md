...
1. **设计思维框架**：开工前要做的美学决定
2. **设计方向池**：参考 `designs/` 目录下的方向文件（用户选择后拿那个当主要执行参考）
3. **字体配对与色彩原则**
4. **反前端 slop 速查**：杀掉 AI 默认审美

> **最重要的改动**：设计方向由用户从 `designs/` 目录中选择。本文档不再提供固定"推荐方向"——方向池在 `designs/README.md` 统一管理。这样设计多样性不依赖你每次的灵感，而是系统性地储备了一套差异化的方向。
...
**`designs/` 目录下的 `.md` 文件是正式的设计方向参考。** 每次执行时用户在 `designs/` 中选择一个方向，你根据该方向的 spec 执行。

本文档不再列举固定方向——方向池应推陈出新，而不是固化在这里。

## 方向选择规则

- 读 `designs/README.md` 了解所有方向的概要
- 根据四定位 + 照片气质匹配合适方向，推荐 2-3 个给用户选择
- 用户选定后，以该方向的 `design.md` 作为 HTML 设计的主执行参考
- **严禁**在所有方向之外再默认用一个"通用布局"——必须从池中选，或用户明确说了要自己设计
